//
//  SCStreamingSessionConstructor.m
//  SoulClash
//
...
@implementation SCStreamingSessionConstructor
{
...
- (void)_generateStreamURLFromServerWithURL:(NSURL *)url
{
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error != nil || response == nil || data == nil) {
            NSLog(@"get SCay json faild, %@, %@, %@", error, response, data);
        ...
        if ([self.delegate respondsToSelector:@selector(SCStreamingSessionConstructor:didGetStreamURL:)]) {
            [self.delegate SCStreamingSessionConstructor:self didGetStreamURL:streamURL];
        }
    }];
    [task resume];
}
